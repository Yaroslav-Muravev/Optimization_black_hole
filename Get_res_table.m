function Get_res_table(res_param)
    %% Parameters
    model_dimension = 2;
    ...
    % model.modelPath('~/Рабочий стол/matlab_kras');
    ...
    tube_air.set('size', {'115 [mm]' '300 [mm]'});
    ...
    pml_down.set('size', {'115 [mm]' '100 [mm]'});
    ...
    pml_up.set('size', {'115 [mm]' '100 [mm]'});
    pml_up.set('pos', {'0' '280 [mm]'});
    pml_up.set('createselection', 'on');
    geom1.run;
        
    r1 = geom1.create('r1', 'Rectangle');
    r1.set('size', [0.001 0.24]);
    r1.set('pos', [0 0.01]);
    ...
    for r_ind = 1:18
        if rings(r_ind) == 1
            ...
            r_obj.set('size', [r_size*0.001 x6]);
            r_obj.set('pos', [0.001 r_pos*0.001]);
    ...
    point_after_structure_x = 0.0575;
    point_after_structure_y = 0.270;
